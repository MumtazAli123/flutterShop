...
logfile="/var/log/agent360-install.log"
...
__Requirements(){
   ...
   An error occured, please check the install log file (/var/log/agent360-install.log)!
...
case $(Linux_Release) in
   Debian*)
      if [ "$(Linux_Version)" -ge 6 ]; then
        ...
        if [[ $? -ne 0 ]]; then
         ...
         if [ "$(ensure_PIP3)" != 'installed' ]  || ! which pip >/dev/null 2>&1; then
            echo "Installing ..."
         ...
         if [ "$(ensure_agent360)" != 'installed' ]; then
           ...
           if [ "$(Linux_Version)" -ge 9 ]; then
            ...
            if [ ! -f /etc/agent360-token.ini ]; then
                command hello360 "$1" /etc/agent360-token.ini --tags="$tags"
            fi
            systemd "$1"
   ...
   CentOS*)
      ...
      echo "Found CentOS..."
        ...
        if [ "$(Linux_Version)" -le 6 ] ; then
           ...
           if [ "$(ensure_PIP)" != 'installed' ]; then
              ...
              if [ "$(cat /etc/*-release | grep -io Oracle| head -1)" != 'Oracle' ]; then
                install "$(get_installer)" libevent
              fi
              echo "Installing agent360 ... "
        ...
        elif [ "$(Linux_Version)" -gt 8 ]; then
            echo "Got CentOS 8..."
            if [ "$(ensure_PIP)" != 'installed' ]; then
               echo "Installing agent360 for RHEL-based 9..."
               echo "Installing python3-pip ..."
               install "$(get_installer)" gcc
               install "$(get_installer)" which
               ...
               systemd "$1"
                    ...
                    echo "Installing agent360 for CentOS 7..."
                     if [ "$(cat /etc/*-release | grep -io RedHat | head -1)" == 'redhat' ]; then
                       echo "Installing RedHat version..."
                       install "$(get_installer)" gcc
                       install "$(get_installer)" @development
                       install "$(get_installer)" which
                       #install "$(get_installer)" https://dl.fedoraproject.org/pub/epel/epel-release-latest-$(rpm -E '%{rhel}').noarch.rpm
                       install "$(get_installer)" python3
                       install "$(get_installer)" python3-setuptools
                       install "$(get_installer)" python3-pip
